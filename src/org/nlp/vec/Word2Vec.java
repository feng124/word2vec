...
public class Word2Vec {
			...
			if (tempCorpus == null) {
				...
				if (!tempDir.exists() && !tempDir.isDirectory()) {
					...
					if (!tempCreated) {
						...
						// System.out.println("临时文件夹创建失败，位于" +
				...
				if (tempCorpus.exists()) {
					...
					// System.out.println("临时文件创建成功，位于" +
	...
	public void training() {
		...
		try {
			...
			// futures.add(threadPool.submit(new Trainer(corpus)));
			corpusQueue.put(corpus);
			logger.info("the task queue has been allocated completely, " + "please wait the thread pool (" + numOfThread
					+ ") to process...");
